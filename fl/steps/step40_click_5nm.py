...
def get_button_rectangle():
    """
    获取5nm选项的矩形区域
    点击范围: (950,480), (980,479), (949,485), (995,484)
...
def click_5nm_option(button_x: int = None, 
                     button_y: int = None,
    ...
    Args:
        button_x: 选项X坐标（如果指定，将使用此坐标）
        button_y: 选项Y坐标（如果指定，将使用此坐标）
        button_rect: 选项矩形区域 (x1, y1, x2, y2)，如果提供，将在矩形内随机选择
        recorder: GUI记录器对象
...
def main():
    """主函数 - 点击选择5nm并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step10_click_5nm.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第十步：点击选择5nm（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击选择5nm - 坐标: ({pos.get('x')}, {pos.get('y')})")
