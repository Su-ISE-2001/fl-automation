...
def get_button_rectangle():
    """
    获取波长扫描按钮的矩形区域
    按钮范围: (736,363), (787,361), (733,369), (787,368)
...
def click_wavelength_scan_button(button_x: int = None, 
                                 button_y: int = None,
...
def main():
    """主函数 - 点击波长扫描按钮并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step61_click_wavelength_scan.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第六十一步：点击波长扫描按钮（记录操作）")
    print("=" * 60)
    ...
    try:
        # 执行点击操作（使用默认按钮矩形区域）
        success = click_wavelength_scan_button(
            recorder=recorder,
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击波长扫描按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
