...
def get_textbox_rectangle():
    """
    获取EX固定波长输入框的矩形区域
    双击范围: (984,616), (1032,617), (983,631), (1032,630)
...
def double_click_ex_wavelength_and_input(textbox_x: int = None, 
                                         textbox_y: int = None,
    ...
    Args:
        ...
        input_value: 双击后要输入的值（如果为None，将从step22结果中读取peak_nm值）
    """
    print("=" * 60)
    print("第十五步：双击EX固定波长输入框，清空并输入数值")
    print("=" * 60)
    
    # 如果未指定input_value，尝试从step22结果中读取peak_nm
    if input_value is None:
        ...
        if result:
            ...
            if 'peak_nm' in result:
                ...
                input_value = str(int(round(peak_nm_value)))  # 四舍五入并转换为整数字符串
                print(f"从step22结果中读取peak_nm值: {peak_nm_value:.6f} -> 输入值: {input_value}")
            elif 'nm' in result:
                nm_value = result['nm']
                input_value = str(int(round(nm_value)))  # 四舍五入并转换为整数字符串
                print(f"从step22结果中读取nm值: {nm_value:.6f} -> 输入值: {input_value}")
            else:
    ...
    try:
        ...
        # 移动鼠标到输入框位置
        print(f"\n移动鼠标到EX固定波长输入框位置 ({textbox_x}, {textbox_y})...")
        ...
        if recorder:
            click_data = {
                "position": {"x": textbox_x, "y": textbox_y},
                "target": "ex_wavelength_textbox",
                "action_type": "double_click",
                "description": "双击EX固定波长输入框"
            }
            if textbox_rect:
        ...
        if recorder:
            input_data = {
                "position": {"x": textbox_x, "y": textbox_y},
                "text": input_value,
                "target": "ex_wavelength_input_field",
...
def main():
    """主函数 - 双击EX固定波长输入框，清空并输入254并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step15_double_click_ex_wavelength.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第十五步：双击EX固定波长输入框，清空并输入254（记录操作）")
    print("=" * 60)
    ...
    try:
        # 执行双击操作并输入254（使用默认EX固定波长输入框矩形区域）
        success = double_click_ex_wavelength_and_input(
            recorder=recorder,
            use_default_rectangle=True,  # 使用默认矩形区域
            input_value="254"  # 输入254
        )
        
        if success:
            print("\n" + "=" * 60)
            print("第十五步完成！EX固定波长输入框应该已经被双击，并已输入254。")
            print("=" * 60)
        else:
            print("\n操作失败，请检查错误信息")
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'double_click':
                    ...
                    print(f"  {i}. 双击EX固定波长输入框 - 坐标: ({pos.get('x')}, {pos.get('y')})")
