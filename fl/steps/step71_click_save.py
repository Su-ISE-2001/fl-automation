...
def get_button_rectangle():
    """
    获取保存按钮的矩形区域
    按钮范围: (1330,334), (1346,377), (1332,352), (1345,353)
...
def click_save_button(button_x: int = None, 
                      button_y: int = None,
...
def main():
    """主函数 - 点击保存按钮保存并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step71_click_save.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第七十一步：点击保存按钮保存（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击保存按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
