...
def get_input_field_rectangle():
    """
    获取文件名输入框的矩形区域
    输入框范围: (766,619), (850,620), (766,631), (823,629)
...
def double_click_and_input_filename(input_x: int = None, 
                                    input_y: int = None,
    ...
    """
    print("=" * 60)
    print("第七十二步：双击文本框输入文本（基于时间的文件名）")
    print("=" * 60)
    ...
    try:
        ...
        if trajectory_recorder and screenshot_before_double_click:
            ...
            x_norm = input_x / screen_width
            y_norm = input_y / screen_height
...
def main():
    """主函数 - 双击文本输入框并输入基于时间的文件名并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step72_input_filename.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第七十二步：双击文本框输入文本（基于时间的文件名）（记录操作）")
    print("=" * 60)
    ...
    try:
        ...
        success = double_click_and_input_filename(
            recorder=recorder,
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'input_text':
                    ...
                    pos = action.get('data', {}).get('position', {})
                    print(f"  {i}. 双击文件名输入框 - 坐标: ({pos.get('x')}, {pos.get('y')})")
