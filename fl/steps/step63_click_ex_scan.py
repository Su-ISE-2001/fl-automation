...
def get_button_rectangle():
    """
    获取EX扫描按钮的矩形区域
    按钮范围: (984,380), (1026,380), (983,384), (1028,386)
...
def click_ex_scan_button(button_x: int = None, 
                          button_y: int = None,
...
def main():
    """主函数 - 点击EX扫描按钮并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step63_click_ex_scan.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第六十三步：点击EX扫描按钮（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击EX扫描按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
