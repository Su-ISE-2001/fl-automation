...
def get_button_rectangle():
    """
    获取xls保存按钮的矩形区域
    按钮范围: (1207,335), (1285,336), (1207,352), (1226,348)
...
def click_xls_save_button(button_x: int = None, 
                          button_y: int = None,
    ...
    try:
        ...
        if trajectory_recorder and screenshot_before:
            ...
            save_click_action(
                ...
                button_y,
                bbox,
                step_instruction="点击xls保存按钮保存xls文件"
            )
...
def main():
    """主函数 - 点击xls保存按钮并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step22_click_xls_save.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第二十二步：点击xls保存按钮保存xls文件（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击xls保存按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
