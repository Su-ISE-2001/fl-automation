...
def get_button_rectangle():
    """
    获取是按钮的矩形区域
    按钮范围: (881,562), (953,562), (882,577), (954,577)
...
def click_yes_button(button_x: int = None, 
                     button_y: int = None,
...
def main():
    """主函数 - 点击是按钮退出软件并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step75_click_yes.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第七十五步：点击是按钮退出软件（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击是按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
