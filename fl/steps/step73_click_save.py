...
def main():
    """主函数 - 点击保存按键并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step73_click_save.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第七十三步：点击保存按键（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击保存按键 - 坐标: ({pos.get('x')}, {pos.get('y')})")
