...
def get_button_rectangle():
    """
    获取联机按钮的矩形区域
    按钮范围: (794,642), (888,642), (793,654), (891,655)
...
def click_button(button_x: int = None, 
                 button_y: int = None,
...
def main():
    """主函数 - 点击联机按钮并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step3_click_button.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第三步：点击联机按钮（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击联机按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
