...
def get_button_rectangle():
    """
    获取设置按钮的矩形区域
    按钮范围: (442,485), (484,483), (437,500), (490,500)
...
def click_settings_button(button_x: int = None, 
                          button_y: int = None,
...
def main():
    """主函数 - 点击设置按钮并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step5_click_settings.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第五步：点击设置按钮（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击设置按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
