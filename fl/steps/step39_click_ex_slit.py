...
def get_button_rectangle():
    """
    获取调整EX狭缝宽按钮的矩形区域
    按钮范围: (1025,437), (1034,436), (1025,442), (1033,442)
...
def click_ex_slit_button(button_x: int = None, 
                          button_y: int = None,
    ...
    try:
        ...
        if recorder:
            click_data = {
                "position": {"x": button_x, "y": button_y},
                "target": "button",
                "button_name": "调整EX狭缝宽按钮",
                "description": "点击调整EX狭缝宽按钮"
            }
            if button_rect:
...
def main():
    """主函数 - 点击调整EX狭缝宽按钮并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step9_click_ex_slit.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第九步：点击调整EX狭缝宽的按钮（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击调整EX狭缝宽按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
