"""
第六十六步：双击EX结束波长输入框，输入结束波长（为步骤64固定波长减20）
记录操作到GUI数据集
"""
import os
import time
...
def get_textbox_rectangle():
    """
    获取EX结束波长输入框的矩形区域
    双击范围: (983,578), (1024,577), (983,588), (1020,590)
...
def compute_ex_end_from_step56(value_field: str = "value", delta: float = 20.0) -> str:
    """
    从步骤56的结果中读取A31的值，并计算结束波长 = 固定波长 - 20
    
    Args:
        value_field: 结果中的字段名（默认"value"）
        delta: 要减去的数值（默认20）
    ...
    if not result:
        print("警告：无法加载step56结果，结束波长使用默认值0")
        return "0"
    
    if value_field not in result:
        print(f"警告：step56结果中未找到字段 {value_field}，结束波长使用默认值0")
        return "0"
    
    raw = result[value_field]
    try:
        fixed_val = float(raw)
        end_val = fixed_val - delta
        input_str = str(int(round(end_val)))
        print(f"从step56结果中读取固定波长: {fixed_val} -> 结束波长 = 固定波长 - {delta} = {end_val} -> 输入值: {input_str}")
        return input_str
    except Exception as e:
        print(f"警告：无法将step56结果的值转换为数值: {raw}，使用默认值0，错误: {e}")
        return "0"


def double_click_ex_end_wavelength_and_input(textbox_x: int = None, 
                                             textbox_y: int = None,
    ...
    """
    双击EX结束波长输入框，清空并输入数值（为步骤64固定波长减20，对应步骤56的A31减20）
    ...
    Args:
        ...
        input_value: 双击后要输入的值（如果为None，将从step56结果中读取A31的value并减20）
    """
    print("=" * 60)
    print("第六十六步：双击EX结束波长输入框，输入结束波长（为固定波长减20）")
    print("=" * 60)
    
    # 如果未指定input_value，尝试从step56结果中读取并减20
    if input_value is None:
        input_value = compute_ex_end_from_step56()
    ...
    try:
        ...
        # 移动鼠标到输入框位置
        print(f"\n移动鼠标到EX结束波长输入框位置 ({textbox_x}, {textbox_y})...")
        ...
        if recorder:
            recorder.add_custom_action("double_click", {
                "position": {"x": textbox_x, "y": textbox_y},
                "target": "ex_end_wavelength_input_field",
                "description": "双击EX结束波长输入框"
            })
        ...
        if trajectory_recorder and screenshot_before_input:
            ...
            save_write_action(
                ...
                textbox_y,
                bbox,
                step_instruction=f"输入EX结束波长值: {input_value}（固定波长减20）"
            )
        ...
        if recorder:
            input_data = {
                "position": {"x": textbox_x, "y": textbox_y},
                "text": input_value,
                "value_source": "step56_A31_minus_20",
                "target": "ex_end_wavelength_input_field",
                "description": f"输入EX结束波长值: {input_value}（固定波长减20，对应A31减20）"
            }
            if textbox_rect:
...
def main():
    """主函数 - 双击EX结束波长输入框并输入数值并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step66_double_click_ex_end_wavelength.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第六十六步：双击EX结束波长输入框，输入结束波长（记录操作）")
    print("=" * 60)
    ...
    try:
        # 执行输入操作（使用默认输入框矩形区域，自动从step56结果中计算结束波长）
        success = double_click_ex_end_wavelength_and_input(
            recorder=recorder,
            use_default_rectangle=True,  # 使用默认矩形区域
            input_value=None  # None表示自动从step56结果中读取并减20
        )
        
        if success:
            print("\n" + "=" * 60)
            print("第六十六步完成！应该已经输入EX结束波长值（固定波长减20）。")
            print("=" * 60)
        else:
            print("\n操作失败，请检查错误信息")
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'input_text':
                    ...
                    print(f"  {i}. 输入EX结束波长值 '{text}' - 坐标: ({pos.get('x')}, {pos.get('y')})")
                elif action_type == 'double_click':
                    pos = action.get('data', {}).get('position', {})
                    print(f"  {i}. 双击EX结束波长输入框 - 坐标: ({pos.get('x')}, {pos.get('y')})")
                elif action_type == 'clear_input':
                    print(f"  {i}. 清空EX结束波长输入框")
