...
def get_button_rectangle():
    """
    获取确定按钮的矩形区域
    按钮范围: (1216,728), (1285,729), (1212,745), (1290,744)
...
def click_confirm_button(button_x: int = None, 
                         button_y: int = None,
...
def main():
    """主函数 - 点击确定按钮并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step67_click_confirm.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第六十七步：点击确定按钮（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击确定按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
