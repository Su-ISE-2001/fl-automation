...
def get_button_rectangle():
    """
    获取调整EM狭缝宽按钮的矩形区域
    按钮范围: (1024,393), (1032,393), (1025,399), (1034,399)
...
def click_em_slit_button(button_x: int = None, 
                          button_y: int = None,
    ...
    try:
        ...
        if recorder:
            click_data = {
                "position": {"x": button_x, "y": button_y},
                "target": "button",
                "button_name": "调整EM狭缝宽按钮",
                "description": "点击调整EM狭缝宽按钮"
            }
            if button_rect:
...
def main():
    """主函数 - 点击调整EM狭缝宽按钮并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step7_click_em_slit.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第七步：点击调整EM狭缝宽的按钮（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击调整EM狭缝宽按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
