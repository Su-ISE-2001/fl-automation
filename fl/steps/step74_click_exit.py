...
def get_button_rectangle():
    """
    获取退出按钮的矩形区域
    按钮范围: (1588,177), (1599,178), (1589,187), (1598,185)
...
def click_exit_button(button_x: int = None, 
                      button_y: int = None,
...
def main():
    """主函数 - 点击退出按钮退出软件并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step74_click_exit.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第七十四步：点击退出按钮退出软件（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击退出按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
