    ...
    try:
        ...
        if recorder:
            click_data = {
                "position": {"x": button_x, "y": button_y},
                "target": "button",
                "button_name": "扫描方式切换按钮",
                "description": "点击扫描方式切换按钮"
            }
            if button_rect:
...
def main():
    """主函数 - 点击扫描方式切换按钮并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step62_click_scan_mode.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第六十二步：点击扫描方式切换按钮（记录操作）")
    print("=" * 60)
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击扫描方式切换按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
