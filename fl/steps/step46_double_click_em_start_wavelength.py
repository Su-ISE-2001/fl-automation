"""
第十六步：双击EM起始波长输入框，清空并输入起始波长（第十五步数值+20）
记录操作到GUI数据集
"""
import os
import time
...
def get_textbox_rectangle():
    """
    获取EM起始波长输入框的矩形区域
    双击范围: (984,407), (1025,443), (982,417), (1034,416)
...
def double_click_em_start_wavelength_and_input(textbox_x: int = None, 
                                               textbox_y: int = None,
    ...
    Args:
        ...
        previous_value: 前一步输入的数值（如果为None，将从step22结果中读取peak_nm值）
        offset: 偏移量（默认20，即起始波长 = 固定波长 + 20）
    """
    # 如果未指定previous_value，尝试从step22结果中读取peak_nm
    if previous_value is None:
        ...
        if result:
            ...
            if 'peak_nm' in result:
                ...
                previous_value = int(round(peak_nm_value))  # 四舍五入并转换为整数
                print(f"从step22结果中读取peak_nm值: {peak_nm_value:.6f} -> 前一步数值: {previous_value}")
            elif 'nm' in result:
                nm_value = result['nm']
                previous_value = int(round(nm_value))  # 四舍五入并转换为整数
                print(f"从step22结果中读取nm值: {nm_value:.6f} -> 前一步数值: {previous_value}")
            else:
    ...
    # 计算要输入的数值
    input_value = str(previous_value + offset)
    
    print("=" * 60)
    print("第十六步：双击EM起始波长输入框，清空并输入起始波长")
    print(f"前一步数值: {previous_value}, 偏移量: {offset}, 输入值: {input_value}")
    print("=" * 60)
    ...
    try:
        ...
        # 移动鼠标到输入框位置
        print(f"\n移动鼠标到EM起始波长输入框位置 ({textbox_x}, {textbox_y})...")
        ...
        if recorder:
            click_data = {
                "position": {"x": textbox_x, "y": textbox_y},
                "target": "em_start_wavelength_textbox",
                "action_type": "double_click",
                "description": "双击EM起始波长输入框"
            }
            if textbox_rect:
        ...
        if trajectory_recorder:
            ...
            save_write_action(
                ...
                textbox_y,
                bbox,
                step_instruction=f"输入起始波长 {input_value} (前一步数值 {previous_value} + {offset})"
            )
        ...
        if recorder:
            input_data = {
                "position": {"x": textbox_x, "y": textbox_y},
                "text": input_value,
                "target": "em_start_wavelength_input_field",
                "previous_value": previous_value,
                "offset": offset,
                "description": f"输入起始波长 {input_value} (前一步数值 {previous_value} + {offset})"
            }
            if textbox_rect:
...
def main():
    """主函数 - 双击EM起始波长输入框，清空并输入起始波长并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step16_double_click_em_start_wavelength.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第十六步：双击EM起始波长输入框，清空并输入起始波长（记录操作）")
    print("=" * 60)
    ...
    try:
        # 执行双击操作并输入起始波长（使用默认EM起始波长输入框矩形区域）
        # 默认前一步数值为254（第十五步的EX固定波长），偏移量为20
        success = double_click_em_start_wavelength_and_input(
            recorder=recorder,
            use_default_rectangle=True,  # 使用默认矩形区域
            previous_value=254,  # 前一步输入的数值（第十五步的EX固定波长）
            offset=20  # 偏移量（起始波长 = 固定波长 + 20）
        )
        
        if success:
            print("\n" + "=" * 60)
            print("第十六步完成！EM起始波长输入框应该已经被双击，并已输入起始波长。")
            print("=" * 60)
        else:
            print("\n操作失败，请检查错误信息")
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'double_click':
                    ...
                    print(f"  {i}. 双击EM起始波长输入框 - 坐标: ({pos.get('x')}, {pos.get('y')})")
                elif action_type == 'clear_input':
                    print(f"  {i}. 清空输入框")
                elif action_type == 'input_text':
                    text = action.get('data', {}).get('text', '')
                    pos = action.get('data', {}).get('position', {})
                    prev_val = action.get('data', {}).get('previous_value', '')
                    offset = action.get('data', {}).get('offset', '')
                    print(f"  {i}. 输入文本 '{text}' (前一步: {prev_val}, 偏移: {offset}) - 坐标: ({pos.get('x')}, {pos.get('y')})")
