...
def get_button_rectangle():
    """
    获取设置按钮的矩形区域
    按钮范围: (442,485), (485,483), (437,500), (490,500)
...
def click_settings_button_fl(button_x: int = None, 
                             button_y: int = None,
    ...
    try:
        ...
        if recorder:
            click_data = {
                "position": {"x": button_x, "y": button_y},
                "target": "button",
                "button_name": "设置按钮（FL流程）",
                "description": "点击设置按钮（FL流程）"
            }
            if button_rect:
...
def main():
    """主函数 - 点击设置按钮（FL流程）并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step60_click_settings.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第六十步：点击设置按钮（FL流程）（记录操作）")
    print("=" * 60)
    ...
    try:
        # 执行点击操作（使用默认按钮矩形区域）
        success = click_settings_button_fl(
            recorder=recorder,
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击设置按钮（FL流程） - 坐标: ({pos.get('x')}, {pos.get('y')})")
