...
def get_button_rectangle():
    """
    获取保存曲线数据按钮的矩形区域
    按钮范围: (1330,334), (1346,337), (1332,352), (1345,353)
...
def click_save_curve_button(button_x: int = None, 
                            button_y: int = None,
    ...
    try:
        ...
        if recorder:
            click_data = {
                "position": {"x": button_x, "y": button_y},
                "target": "button",
                "button_name": "保存曲线数据按钮",
                "description": "点击保存曲线数据按钮"
            }
            if button_rect:
...
def main():
    """主函数 - 点击保存曲线数据按钮并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step57_click_save_curve.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第五十七步：点击保存曲线数据按钮（记录操作）")
    print("=" * 60)
    ...
    try:
        # 执行点击操作（使用默认按钮矩形区域）
        success = click_save_curve_button(
            recorder=recorder,
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击保存曲线数据按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
