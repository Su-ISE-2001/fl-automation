...
def get_button_rectangle():
    """
    获取仪器按钮的矩形区域
    按钮范围: (702,330), (810,328), (706,344), (819,344)
...
def click_instrument_button(button_x: int = None, 
                            button_y: int = None,
...
def main():
    """主函数 - 点击仪器按钮并记录操作"""
    # 初始化GUI记录器
    output_file = "FL_step6_click_instrument.json"
    recorder = GUIRecorder(output_file)
    
    print("\n" + "=" * 60)
    print("第六步：点击仪器按钮（记录操作）")
    print("=" * 60)
    ...
    try:
        # 执行点击操作（使用默认按钮矩形区域）
        success = click_instrument_button(
            recorder=recorder,
        ...
        if recorder.actions:
            ...
            for i, action in enumerate(recorder.actions, 1):
                ...
                if action_type == 'click':
                    ...
                    print(f"  {i}. 点击仪器按钮 - 坐标: ({pos.get('x')}, {pos.get('y')})")
